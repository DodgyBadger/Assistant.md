...
Approved for incremental implementation. Slices 0 through 4 are complete, and Slice 5 is the next experimental slice. The selected first runtime hypothesis is cumulative Jev-gated maintenance: frequent cheap decision checks control less frequent generative reconciliation and verification. This plan defines the first session-memory slice and intentionally leaves cross-session recall, vault recall, capability scouting, and research-artifact evaluation out of scope.
...
24. Decision progress and map coverage are distinct. `decision_checked_through_sequence_index` records scheduling and diagnostics only; only a verified, atomically committed map revision advances `updated_through_sequence_index` and resets the cumulative pending range.
25. Post-author verification is bounded. A proposed revision is checked by the decision model for field-level support, completeness, lifecycle correctness, and current salience against the same frozen cumulative source range. One configured regeneration may be attempted; failure leaves the prior map and cumulative delta intact for retry rather than accepting a low-confidence revision or looping indefinitely.
...
The maintenance pipeline should process only canonical raw messages newer than the selected map revision's high-water mark. That range is cumulative until a replacement map is verified and committed; classifier checks do not consume it.
...
completed persisted turn
  ...
  -> run parallel Jev checks over the proposed map and the same frozen evidence
  -> optionally regenerate once from failed verification dimensions
...
5. Ask the decision model, in parallel where the adapter permits, whether attention and each map collection remain adequate in light of the entire frozen cumulative delta, together with global questions for newly introduced concepts and material omissions. Only a high-confidence stable result across every required question may skip authoring. Persist raw probabilities, thresholds, resolved model identity, usage, latency, and `decision_checked_through_sequence_index`; do not advance map coverage or clear the delta.
6. Trigger generative reconciliation when any required stability score falls below its calibrated skip threshold or an independent cumulative turn/token ceiling is reached. The hard ceiling prevents indefinite classifier growth and cumulative micro-change from remaining unincorporated. If the classifier fails or becomes unavailable, leave the prior map and pending range intact, record a retryable failure, and keep ordinary chat on the existing fallback behavior; do not bypass the required verifier by authoring an unverified revision.
7. Validate and apply the author's typed operations against the frozen evidence, then send the complete proposed map, prior map, and same cumulative delta through parallel decision checks for source support, omissions, lifecycle correctness, and current salience. If verification falls below its calibrated acceptance threshold, permit at most one configured regeneration using the failed dimensions as feedback. A rejected or failed proposal leaves the prior map and cumulative delta intact.
8. Persist a verified revision only if its predecessor and frozen source view still match; otherwise discard or retry against the newer state. A successful compare-and-swap advances map coverage and resets pending accounting to messages after the committed watermark.
...
Represent pre-author adequacy detection as one typed output model with independent stability fields for attention, goals, work items, decisions, constraints, commitments, open questions, artifacts, and observations, plus global fields for newly introduced concepts and material omissions. Put the shared task framing in agent instructions, put one atomic question in each field description, and pass the accepted current map plus the entire cumulative canonical delta since its coverage watermark as the state being judged. Every field evaluates the same frozen delta; a prior confident skip does not shorten the next request. Do not put questions in the run prompt or a system-prompt history item because Pydantic AI sends those as state, not as Jev questions.

Prefer bounded `float` fields from `0` to `1` expressing confidence that the corresponding field remains adequate without reconciliation. Pydantic AI maps each such field to Jev's unrounded probability of yes, allowing AssistantMD to require a high calibrated confidence before skipping the author and to tune thresholds separately by map dimension. Any low-confidence or uncertain required field triggers authoring; this intentionally favors false-positive author calls over false-negative stale maps. A `bool` output would round through one model-level boolean threshold and is less suitable for offline tuning or asymmetric error costs.
...
The configured decision classifier is the primary control loop: it decides whether every map field is still adequate against accumulated changes and therefore whether generative reconciliation can be skipped. It does not write map prose, validate authority, apply patches, or select canonical sources. A generative Pydantic AI agent remains responsible for proposing typed map patch operations when any field fails the stability gate or the hard ceiling fires, and ordinary code validates and applies those operations.

After deterministic patch application, use a second typed Jev contract to judge the complete proposed map against the prior map and the identical frozen cumulative delta. Its parallel fields cover support, material omissions, lifecycle/adoption/verification correctness, attention, and current salience. The verifier may accept the proposal or supply failed dimensions to one bounded regeneration attempt; it never mutates the map directly. If the final verification remains below threshold, no revision is committed and no evidence is consumed. Pre-author skip thresholds and post-author acceptance thresholds are separate calibrated policies.
...
The primary Slice 5 experiment is now the coupled cumulative Jev pipeline. At each candidate interval `X`, Jev judges whether every current map field remains adequate against the entire unmapped canonical delta. A confident stable result leaves that delta pending, so the next request contains the original accepted map, all previously checked messages, and the newly completed messages. Any field below its calibrated high-confidence skip threshold invokes the author over that same cumulative range. An independent maximum pending-turn or token ceiling forces authoring regardless of classifier output.

Test candidate values of `X`, field questions, and skip thresholds first on labelled cumulative prefixes, optimizing for recall of consequential changes before avoided author calls. Compare the selected Jev gate with reconcile-every-eligible-batch and hard-cadence-only baselines. Measure false stable decisions, accumulated micro-change detection, maximum freshness lag, classifier and author call counts, cumulative classifier input growth, combined tokens and latency, and final semantic map quality. A confident stable decision never resets or truncates the source range.

Once the gate is calibrated, add post-author Jev verification over the proposed map, prior map, and identical frozen delta. Compare no verifier, verifier-only rejection, and one bounded verifier-guided regeneration. Measure unsupported claims, missed concepts, lifecycle/adoption/verification errors, salience, accepted-map quality, rejection rate, retry yield, and total cost. Do not permit repeated retries or advance map coverage after a rejected proposal.
...
**Hypothesis:** Frequent cumulative decision checks can keep the map timely while avoiding most generative author calls, and bounded verification can reject materially incomplete or unsupported revisions without affecting chat reliability.

**Build:** Add disabled-by-default `live_session_memory_mode` with `off` and `observe` initially, plus configurable `live_session_memory_author_model`, `live_session_memory_decision_model`, eligibility interval, pre-author skip thresholds, post-author acceptance thresholds, maximum pending-turn/token ceiling, and maximum regeneration count fixed initially at zero or one. Validate generative text capability for the author and decision capability for the classifier with independent provider, credential, and resolved-model diagnostics. Add `core/memory/session_map/service.py`, compose one instance in `RuntimeContext`, and submit work through `ExecutionTaskRunner.start_background(...)` with an explicit session-memory task spec, inherited execution authority, timeout and lifecycle hooks, a keyed per-session execution gate, and the shared bounded-concurrency policy. The service freezes one cumulative source range and runs its parallel Jev gate, optional author, deterministic validator, and parallel Jev verifier inside that tracked task. The successful-turn hook only durably advances pending counters and dispatches or coalesces work; it never calls a memory model inline and never uses a parallel background-task path. In `observe`, persist classifier decisions, verified map revisions, retry outcomes, and diagnostics without injecting the map or changing compaction output. Keep core lifecycle ownership outside generic scripts.

**Verify:** Exercise cumulative skip checks, repeated stable decisions, accumulation into a later trigger, newly introduced concepts absent from the old map, hard-ceiling authoring, author acceptance, verifier rejection, one bounded regeneration, final rejection, coalescing, background failures, timeout and cancellation hooks, retry/catch-up, process restart, concurrent turns, stale-task rejection, and missing or incompatible model fallback. Assert that skips advance only `decision_checked_through_sequence_index`; accepted revisions alone advance map coverage and clear the covered pending range. Assert through task snapshots and validation events that classifier, author, and verifier calls occur only inside the tracked session-memory execution task, parallel decision work shares that task's lifecycle, concurrent work for one session queues behind its execution gate, global memory-model work respects its bounded-concurrency lane, task authority matches the originating session action, and no untracked coroutine or inline post-turn model call occurs. Compare shadow maps with forced audits and labelled samples while users continue to see existing behavior.
...
- False-stable rate and consequential-change recall for cumulative per-field Jev checks at each candidate eligibility interval and skip threshold.
- Combined classifier, author, verifier, and bounded-regeneration cost and freshness under cumulative Jev gating plus the hard ceiling.
...
- Verifier rejection rate, retry yield, final rejection rate, and semantic quality of accepted versus unverified proposals.
...
- The eligibility interval `X`, per-field pre-author skip thresholds, post-author acceptance thresholds, and cumulative hard turn/token ceiling.
- Whether verifier-guided regeneration should be disabled or permit exactly one retry, and which failed decision dimensions may be returned as author feedback.
...
Continue with Slice 5 by building a labelled cumulative-prefix experiment around the existing Jev decision runtime. Compare candidate eligibility intervals, field-adequacy questions, global missing-concept checks, and high-confidence skip thresholds while preserving the full delta after every skip. Then run the selected gate with the existing author, add bounded post-author Jev verification, and use the span harness to choose the independent hard ceiling and characterize the author ranges the gate actually produces. No runtime maintenance hook, prompt injection, or compaction change is admissible until the cumulative gate, author, and verifier quality/cost surface is understood.
