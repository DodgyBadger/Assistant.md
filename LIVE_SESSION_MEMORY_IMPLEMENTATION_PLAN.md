...
#### Full-Path Simulated Conversation Probe

Add an opt-in experimental scenario that uses Terra for two distinct roles: a harness-owned user simulator and the ordinary Assistant.md chat agent. The simulator receives a fixed staged brief plus the accumulated visible transcript and emits the next natural user message as typed output. Every assistant turn enters through the public chat-task API, so chat context assembly, canonical persistence, post-turn accounting, tracked session-memory execution, Jev gating, optional Terra authoring, and durable map inspection all run through their production control paths. The default probe runs twelve turns with a memory checkpoint every three turns and records the transcript, task outcomes, maintenance states, and map revisions for semantic review.

The scenario may read the configured development model/provider definitions and copy only the required secrets into its isolated encrypted validation root. OpenAI OAuth transfer is limited to a sufficiently long-lived access token with the refresh token removed, preventing an experiment from rotating or invalidating the development connection. The probe remains outside `integration/core`, spends live quota, makes no deterministic assertions about model prose, and fails only on structural failures such as an incomplete chat turn, an untracked or unrecovered memory attempt, a cancelled or timed-out memory task, missing durable state, an unbounded pending tail, or failure to produce any committed map after deliberately consequential changes. A transient rejected author patch is recorded as experimental evidence and is acceptable only when a later ordinary turn schedules a successful catch-up and clears the durable error; the harness must never silently count the failed attempt as a successful checkpoint.

The first twelve-turn Terra/Terra run established the full production path and produced three committed revisions. Jev requested authoring at every tested three-turn checkpoint, so this deliberately change-dense script is useful for lifecycle and author-quality testing but not for estimating skip rate. The third authored patch failed deterministic `SessionMapError` validation; the next ordinary chat turn automatically scheduled a tracked catch-up task, and that task committed successfully while chat continued. Two turns then remained as an exact pending tail below the next eligibility boundary, demonstrating both failure isolation and the expected map-plus-tail freshness model. Semantic review found strong retention of the corrected budget, removed site, two-site shortlist, evidence guardrail, incentive conditions, blocker resolution, and outstanding temperature dataset, but also two author-quality concerns for later tuning: no explicit decision entry was created, and the council-briefing artifact was marked complete before the user's later direction that it was ready for drafting. Treat these as probe findings, not generalized rates or reasons to expand the schema prematurely.
...
Optional observability hardening: wrap each tracked session-memory attempt in one enclosing telemetry span carrying its execution task ID, session ID, vault, source range, and outcome so the Jev gate, optional author call, and commit appear as one correlated trace. This is diagnostic convenience only. Product-level inspection and auditing of session maps, revisions, decisions, source evidence, pending state, and failures must use the durable application-owned records and must not depend on Logfire or any other external telemetry service.
