...
Approved for incremental implementation. Slices 0 through 4 are complete, and Slice 5 is the next experimental slice. This plan defines the first session-memory slice and intentionally leaves cross-session recall, vault recall, capability scouting, and research-artifact evaluation out of scope.
...
- Live session memory is disabled by default. With it disabled, or without the compatible and ready model roles required by the selected maintenance policy, chat and compaction use the current recovery-card behavior unchanged. An author model is always required; a decision model is required only for a policy that enables semantic gating.
...
15. Session-memory orchestration keeps provider-neutral decision classification and generative authoring as separate configured roles. Jev is the first optional semantic-gating adapter, while Terra and Sol are author candidates through the ordinary text-model factory; none is a permanent dependency of the map, scheduling, persistence, or compaction contracts. A deterministic compaction-aligned policy does not call a decision model.
...
Before further prompt tuning or a broad model/effort sweep, run a span-size screening experiment against the ignored production-derived long sessions. Replay identical source prefixes under at least three regimes: small incremental windows, medium accumulated token windows, and the natural compaction-scale or full-unmapped prefix approaching the existing `150,000`-token boundary. Include the current recovery-card result and one single-pass full-prefix map as baselines. Hold the V7 schema, prompt, author model, and explicit thinking effort constant within this comparison. Score final semantic fidelity, current-handoff accuracy, unsupported promotion, stale-active state, evidence precision, map density, revision-to-revision churn, input/output usage, latency, and total authoring cost over the whole source prefix—not merely per call. A long-span run must use bounded source representation only if that representation is itself part of the tested policy; silently truncating the prefix would invalidate the comparison.

Then screen author configuration on the most promising one or two span regimes rather than exhaustively crossing every factor. Compare Terra and Sol at explicit `low` and `medium` thinking effort so provider-default reasoning is not an uncontrolled variable. Retain the current default-effort runs as development evidence but do not treat `default` as equivalent across models. Repeat only configurations that pass the typed and semantic gates, then use a new untouched holdout for any selection claim.

Finally, evaluate the coupled scheduler rather than grafting the existing Jev result onto authoring by assumption. At a deterministic minimum accumulated-turn or token threshold, Jev judges the current map against the entire unmapped canonical delta. A negative result leaves that delta pending; it does not select or discard messages, and the next check may therefore present a larger and potentially noisier authoring span. Compare deterministic authoring at the selected span, Jev-gated authoring with the same minimum threshold, and Jev plus an independent hard maximum-turn/token trigger. Measure final map quality, maximum freshness lag, classifier and author call counts, combined tokens and latency, and the semantic content of deferred deltas. This coupled experiment, not the earlier isolated classifier accuracy, determines whether Jev improves the production quality/cost frontier.

If compaction-scale or full-prefix authoring matches or exceeds incremental quality, prefer the simpler compaction-aligned first runtime policy. Its first map is authored from the canonical prefix reaching the compaction threshold; later revisions reconcile the prior map against the next exact unmapped span. The retained raw tail supplies between-boundary continuity, the existing compaction execution task supplies lifecycle ownership, and Jev remains outside the critical path. Only evidence that materially fresher mid-cycle maps improve continuation should justify adding semantic-gated background maintenance.
...
**Hypothesis:** The selected scheduler and authoring pipeline can maintain a sufficiently timely map without affecting chat reliability; the first policy may be deterministic and compaction-aligned if Slice 5 shows no benefit from mid-cycle semantic gating.

**Build:** Add disabled-by-default `live_session_memory_mode` with `off` and `observe` initially, plus configurable `live_session_memory_author_model` and an optional `live_session_memory_decision_model` used only by a selected semantic-gating policy. Validate generative text capability for the author and, when enabled, decision capability for the classifier, with independent provider, credential, and resolved-model diagnostics. Implement only the maintenance policy selected by Slice 5 evidence. A compaction-aligned policy reuses the existing tracked compaction task and does not add a post-turn model call. If semantic-gated background maintenance wins, add `core/memory/session_map/service.py`, compose one instance in `RuntimeContext`, and submit work through `ExecutionTaskRunner.start_background(...)` with an explicit session-memory task spec, inherited execution authority, timeout and lifecycle hooks, a keyed per-session execution gate, and the shared bounded-concurrency policy. The service may coalesce durable pending ranges before or inside that tracked task, but the successful-turn hook only records and dispatches work; it never calls the classifier or author inline and never uses a parallel background-task path. In `observe`, persist map revisions and diagnostics without injecting the map or changing compaction output. Keep core lifecycle ownership outside generic scripts.

**Verify:** Exercise successful updates, coalescing, hard and semantic triggers, negative classifier results, background failures, timeout and cancellation hooks, retry/catch-up, process restart, concurrent turns, stale-task rejection, and missing or incompatible model fallback. Assert through task snapshots and validation events that classifier and author calls occur only inside the tracked session-memory execution task, concurrent work for one session queues behind its execution gate, global memory-model work respects its bounded-concurrency lane, task authority matches the originating session action, and no untracked coroutine or inline post-turn model call occurs. Compare shadow maps with forced audits and labelled samples while users continue to see existing behavior.
...
- Combined classifier-plus-author cost and freshness under deterministic cadence, Jev gating, and Jev plus hard-trigger scheduling.
...
- Which accumulated turn/token span should make maintenance eligible, ranging from small incremental updates through the existing compaction-scale boundary, and whether Jev improves on the selected deterministic cadence once deferred deltas are carried forward intact.
...
Continue with Slice 5 by screening authoring span before further prompt tuning. Use the ignored production-derived long sessions to compare incremental, medium accumulated, and natural compaction-scale/full-prefix reconciliation under one frozen author configuration, then screen model and explicit thinking effort only on the promising span regimes. After that, run the coupled Jev scheduler with deferred deltas and hard triggers. No runtime maintenance hook, prompt injection, or compaction change is admissible until this quality/cost surface is understood.
