...
- Live session memory is disabled by default. With it disabled, or without compatible and ready decision and authoring models, chat and compaction use the current recovery-card behavior unchanged.
...
14. The live-memory compaction path is admissible only when the configured decision alias declares decision capability, the configured author alias declares generative text capability, both providers and credentials are ready, and a committed map revision covers every canonical message to be removed from effective context. If any condition fails, compaction uses the existing recovery-card path wholesale.
15. Session-memory orchestration keeps provider-neutral decision classification and generative authoring as separate configured roles. Jev is the first decision adapter, while Terra and Sol are author candidates through the ordinary text-model factory; none is a permanent dependency of the map, scheduling, persistence, or compaction contracts.
...
**Build:** Add disabled-by-default `live_session_memory_mode` with `off` and `observe` initially, plus independently configurable `live_session_memory_decision_model` and `live_session_memory_author_model`. Validate decision capability for the former and generative text capability for the latter, with independent provider, credential, and resolved-model diagnostics. Add `core/memory/session_map/service.py`, compose one instance in `RuntimeContext`, and submit post-commit maintenance through `ExecutionTaskRunner.start_background(...)` with an explicit session-memory task spec, inherited execution authority, timeout and lifecycle hooks, a keyed per-session execution gate, and the shared bounded-concurrency policy. The service may coalesce durable pending ranges before or inside that tracked task, but the successful-turn hook only records and dispatches work; it never calls the classifier or author inline and never uses a parallel background-task path. In `observe`, atomically record pending work, deliver UI completion, and persist map revisions and diagnostics. Never inject the map or change compaction. Keep core lifecycle ownership outside generic scripts.
...
Continue with Slice 5, the offline map-authoring experiment. Slices 0 through 4 have frozen the evaluation contract, established decision-model configuration, added a reusable typed classifier runtime, implemented the deterministic map and persistence foundation, and rejected per-turn classification on cost despite strong v2 discrimination. Authoring quality and cost must now be measured independently through validated typed patches; no runtime maintenance hook, prompt injection, or compaction change is yet admissible.
