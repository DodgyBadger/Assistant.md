(function configurationFeature(window, document) {
    ...
    async function startMcpOAuth(card, endpoint) {
        ...
        try {
            ...
            const owner = createOAuthPoll(resources.mcpOAuthPolls, id, 10 * 60 * 1000);
    ...
    async function pollMcpOAuthStatus(id, endpoint, owner) {
        while (ownsOAuthPoll(resources.mcpOAuthPolls, id, owner)) {
            if (Date.now() >= owner.deadline) {
                if (finishOAuthPoll(resources.mcpOAuthPolls, id, owner)) {
            ...
            try {
                ...
                const payload = await safeJson(response);
                if (!ownsOAuthPoll(resources.mcpOAuthPolls, id, owner)) return;
                if (response.ok && (payload?.connected || payload?.status === 'failed' || payload?.status === 'expired')) {
                    if (!finishOAuthPoll(resources.mcpOAuthPolls, id, owner)) return;
                ...
                if (error.name === 'AbortError' || !ownsOAuthPoll(resources.mcpOAuthPolls, id, owner)) return;
                // A transient status failure should not cancel the browser flow.
            }
            if (!await waitForOAuthPoll(resources.mcpOAuthPolls, id, owner, 2000)) return;
        }
    }
